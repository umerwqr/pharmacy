 ...
 */
const validateSingleStringParams = (param: string) => {
  return check(param)
    .exists()
 ...
 */
const validateStringParams = (param: string) => {
  return check(param)
    .exists()
...
const validateAlphanumericParams = (param: string) => {
  return check(param)
    .exists()
    .withMessage(`${param} should exist.`)
    .isString()
 ...
 * it should not empty and should match the Regex value
 */
const validateOptionalStringParams = (param: string) => {
  return check(param)
    .optional()
    .notEmpty()
    .withMessage(`${param} should not Empty.`)
 ...
 * it should not empty and should match the Regex value
 */
const validateNumberParams = (param: number) => {
  return check(param)
    .exists()
    .withMessage(`${param} should Exists.`)
    .isNumeric()
 ...
 * it should not empty and should match the Regex value
 */
const validateOptionalNumberParams = (param: number) => {
  return check(param)
    .optional()
    .isNumeric()
    .withMessage(`${param} should only be Numbers.`)
 ...
 * it should not empty and should match the Regex value
 */
const validateDecimalParams = (param: number) => {
  return check(param)
    .exists()
    .withMessage(`${param} should Exist.`)
    .isNumeric()
 ...
 *  it should not empty and should match the Regex value
 */
const validateFloatParams = (param: number) => {
  return check(param)
    .exists()
    .withMessage(`${param} should exist.`)
    .isNumeric()
 ...
 */
const validateUsername = (param: string) => {
  ...
  return check(param)
    ...
    .withMessage(
      `${param} should be between ${minChars} and ${maxChars} characters.`
    )
    .matches(/^[A-Za-z0-9_.-]+$/)
    .withMessage(
      `${param} can only contain Alphabets, Numbers, '_', '.', '-' characters.`
    );
};
  ...
  return check(param)
    ...
    .custom((value: string) => {
      try {
        ...
        if (!isStrongPassword) {
          throw new Error(
            "Password should be a strong password containing at least 8 characters, including at least one uppercase letter, one lowercase letter, one number, and any special character."
          );
        }
        ...
        throw new Error(
          "Password is not correctly encrypted or does not meet strength requirements"
        );
      }
    });
};
 ...
 */
const validateBooleanParam = (param: string) => {
  return check(param)
    .exists()
    .withMessage(`${param} should Exist.`)
    .isBoolean()
 ...
 * and is a date value.
 */
const validateDateParam = (param: string) => {
  return check(param)
    .exists()
    .withMessage(`${param} should Exist.`)
 ...
 */
const validateDatetime = (param: string) => {
  return check(param)
    .exists()
    .withMessage(`${param} should Exist.`)
    .isString()
 ...
 * and is a date value.
 */
const validateOptionalDateParam = (param: string) => {
  return check(param)
    .optional()
    .isDate({ format: "YYYY-MM-DD" })
    .withMessage(`${param} should be in the format YYYY-MM-DD.`);
 ...
 */
const validateStringArrayParams = (param: string, arrValue: any) => {
  return check(param)
    .exists()
    .withMessage(`${param} should Exists.`)
    .isString()
    ...
    .isIn(arrValue)
    .withMessage(
      `${param} should be from the following values : '${arrValue}'`
    );
};
 ...
 * it should be present in the array value.
 */
const validateOptionalStringArrayParams = (param: string, arrValue: any) => {
  return check(param)
    .optional()
    .withMessage(`${param} should Exists.`)
    .isString()
 ...
 */
const validateNumberArrayParams = (param: string) => {
  return check(param)
    .exists()
    .withMessage(`${param} should exist.`)
    .isArray()
 ...
 */
const validateImageFileName = (param: string) => {
  ...
  return check(param)
    ...
    .custom((value: any) => {
      ...
      if (!allowedTypes.includes(fileExtension)) {
        throw new Error(
          `${param} should be one of the following types: ${allowedTypes.join(
            ", "
          )}.`
        );
      }
 ...
 */
const validatePasswordEncryption = (param: string) => {
  return check(param)
    .exists()
    .withMessage(`${param} should exist.`)
    .isString()
 ...
 */
const validatePercentage = (param: string) => {
  return check(param)
    .exists()
    .withMessage(`${param} should exist.`)
 ...
 * and falls between 0 and 100.
 */
const validateOptionalPercentage = (param: string) => {
  return check(param)
    .optional()
    .isFloat({ min: 0, max: 100 })
    .withMessage(`${param} should be a number between 0 and 100.`)
  ...
  return body("payments")
    ...
    .custom((payments: any) => {
      payments.forEach((payment: any, index: number) => {
        ...
        if (
          !["bank", "usdt", "wallet", "vreit"].includes(payment.paymentType)
        ) {
          throw new Error(
            `${payments[index].paymentType} must be one of "bank", "usdt", "wallet", or "vreit"`
          );
        }
        ...
        if (["bank", "usdt"].includes(payment.paymentType)) {
          if (!payment.filename) {
            throw new Error(
              `${payments[index].filename} is required when paymentType is "bank" or "usdt"`
            );
          }
          ...
          if (!allowedTypes.includes(fileExtension)) {
            throw new Error(
              `${
                payments[index].filename
              } should be one of the following types: ${allowedTypes.join(
                ", "
              )}`
            );
          }
